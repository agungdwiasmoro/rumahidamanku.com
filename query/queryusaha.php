...
mysql_select_db($database_koneksi, $koneksi);
//mysql_select_db($database_konekprof, $konekprof);
$query_konekprofhome = "SELECT * FROM profilpromo WHERE namausaha = '$namausaha' AND provinsi = '$provinsi' AND kodkab = '$kodkab' ORDER BY id_profilpromo ASC";
...
$maxRows_konekiventvid = 1;
$pageNum_konekiventvid = 0;
if (isset($_GET['pageNum_konekiventvid'])) {
  $pageNum_konekiventvid = $_GET['pageNum_konekiventvid'];
}
$startRow_konekiventvid = $pageNum_konekiventvid * $maxRows_konekiventvid;

mysql_select_db($database_koneksi, $koneksi);
//mysql_select_db($database_konekivent, $konekivent);
$query_konekiventvid = "SELECT * FROM iventpromo WHERE namausaha = '$namausaha' AND status = 'on' AND provinsi = '$provinsi' AND kodkab = '$kodkab' AND jenis = 'video' ORDER BY id_iventpromo DESC";
$query_limit_konekiventvid = sprintf("%s LIMIT %d, %d", $query_konekiventvid, $startRow_konekiventvid, $maxRows_konekiventvid);
$konekiventvid = mysql_query($query_limit_konekiventvid, $koneksi) or die(mysql_error());
//$konekiventprodiy = mysql_query($query_limit_konekiventprodiy, $konekivent) or die(mysql_error());
$row_konekiventvid = mysql_fetch_assoc($konekiventvid);

if (isset($_GET['totalRows_konekiventvid'])) {
  $totalRows_konekiventvid = $_GET['totalRows_konekiventvid'];
} else {
  $all_konekiventvid = mysql_query($query_konekiventvid);
  $totalRows_konekiventvid = mysql_num_rows($all_konekiventvid);
}
$totalPages_konekiventvid = ceil($totalRows_konekiventvid/$maxRows_konekiventvid)-1;

$queryString_konekiventvid = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_konekiventvid") == false && 
        stristr($param, "totalRows_konekiventvid") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_konekiventvid = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_konekiventvid = sprintf("&totalRows_konekiventvid=%d%s", $totalRows_konekiventvid, $queryString_konekiventvid);
...
$query_konekproddiy = "SELECT * FROM produkpromo WHERE namausaha = '$namausaha' && status = 'on' AND provinsi = '$provinsi' AND kodkab = '$kodkab' ORDER BY id_produkpromo DESC";
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekdprri = "SELECT * FROM profilcaleg WHERE kodkab = '$kodkab' AND namacaleg = '$namacaleg' AND status = 'ok' ORDER BY id_calegkodkab DESC";
//$konekdprd = mysql_query($query_konekdprd, $konekdprdkodkab) or die(mysql_error());
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekdprdsatu = "SELECT * FROM profilcaleg WHERE kodkab = '$kodkab' AND namacaleg = '$namacaleg' AND status = 'ok' ORDER BY id_calegkodkab DESC";
//$konekdprd = mysql_query($query_konekdprd, $konekdprdkodkab) or die(mysql_error());
...
//mysql_select_db($database_konekdprdkodkab, $konekdprdkodkab);
mysql_select_db($database_koneksi, $koneksi);
$query_konekdprd = "SELECT * FROM profilcaleg WHERE kodkab = '$kodkab' AND namacaleg = '$namacaleg' AND status = 'ok' ORDER BY id_calegkodkab DESC";
//$konekdprd = mysql_query($query_konekdprd, $konekdprdkodkab) or die(mysql_error());
$konekdprd = mysql_query($query_konekdprd, $koneksi) or die(mysql_error());
$row_konekdprd = mysql_fetch_assoc($konekdprd);
$totalRows_konekdprd = mysql_num_rows($konekdprd);
