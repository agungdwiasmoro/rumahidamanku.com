...
$query_indonesia = "SELECT * FROM indonesia ORDER BY id_input DESC";
$query_limit_indonesia = sprintf("%s LIMIT %d, %d", $query_indonesia, $startRow_indonesia, $maxRows_indonesia);
$indonesia = mysql_query($query_limit_indonesia, $koneksi) or die(mysql_error());
...
$maxRows_alone = 1;
$pageNum_alone = 0;
if (isset($_GET['pageNum_alone'])) {
  $pageNum_alone = $_GET['pageNum_alone'];
}
$startRow_alone = $pageNum_alone * $maxRows_alone;


$query_alone = "SELECT * FROM alone ORDER BY id_alone DESC";
$query_limit_alone = sprintf("%s LIMIT %d, %d", $query_alone, $startRow_alone, $maxRows_alone);
$alone = mysql_query($query_limit_alone, $koneksi) or die(mysql_error());
$row_alone = mysql_fetch_assoc($alone);

if (isset($_GET['totalRows_alone'])) {
  $totalRows_alone = $_GET['totalRows_alone'];
} else {
  $all_alone = mysql_query($query_alone);
  $totalRows_alone = mysql_num_rows($all_alone);
}
$totalPages_alone = ceil($totalRows_alone/$maxRows_alone)-1;
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (stristr($param, "pageNum_alone") == false && 
        stristr($param, "totalRows_alone") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_alone = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_alone = sprintf("&totalRows_alone=%d%s", $totalRows_alone, $queryString_alone);
...
$queryString_indonesia = sprintf("&totalRows_indonesia=%d%s", $totalRows_indonesia, $queryString_indonesia);
