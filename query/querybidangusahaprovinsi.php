...
mysql_select_db($database_koneksi, $koneksi);
$query_konekprofhome = "SELECT * FROM profilpromo WHERE provinsi = '$provinsi' and status= 'on' && bidangusaha= '$bidangusaha'  ORDER BY id_profilpromo ASC";
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekinformasi = "SELECT * FROM info WHERE provinsi= '$provinsi' && bidangusaha= '$bidangusaha' && jenis= 'photo' && status= '$status' ORDER BY id_info DESC";
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekjawatengah = "SELECT * FROM profilkabkod WHERE provinsi = '$provinsi' && kodkab='none' ORDER BY id_profkabkod DESC";
...
$maxRows_koneklokerjateng = 12;
$pageNum_koneklokerjateng = 0;
if (isset($_GET['pageNum_koneklokerjateng'])) {
  $pageNum_koneklokerjateng = $_GET['pageNum_koneklokerjateng'];
}
$startRow_koneklokerjateng = $pageNum_koneklokerjateng * $maxRows_koneklokerjateng;

mysql_select_db($database_koneksi, $koneksi);
$query_koneklokerjateng = "SELECT * FROM lokerpromo WHERE provinsi = '$provinsi' && status = '$status' && bidangusaha = '$bidangusaha' ORDER BY id_lokerpromo DESC";
$query_limit_koneklokerjateng = sprintf("%s LIMIT %d, %d", $query_koneklokerjateng, $startRow_koneklokerjateng, $maxRows_koneklokerjateng);
$koneklokerjateng = mysql_query($query_limit_koneklokerjateng, $koneksi) or die(mysql_error());
$row_koneklokerjateng = mysql_fetch_assoc($koneklokerjateng);

if (isset($_GET['totalRows_koneklokerjateng'])) {
  $totalRows_koneklokerjateng = $_GET['totalRows_koneklokerjateng'];
} else {
  $all_koneklokerjateng = mysql_query($query_koneklokerjateng);
  $totalRows_koneklokerjateng = mysql_num_rows($all_koneklokerjateng);
}
$totalPages_koneklokerjateng = ceil($totalRows_koneklokerjateng/$maxRows_koneklokerjateng)-1;

$maxRows_konektipsmotor = 2;
...
mysql_select_db($database_koneksi, $koneksi);
$query_konektipsmotor = "SELECT * FROM tipsmotor WHERE bidangusaha = 'wisata' ORDER BY id_tipsmotor DESC";
...
$queryString_koneklokerjateng = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_koneklokerjateng") == false && 
        stristr($param, "totalRows_koneklokerjateng") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_koneklokerjateng = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_koneklokerjateng = sprintf("&totalRows_koneklokerjateng=%d%s", $totalRows_koneklokerjateng, $queryString_koneklokerjateng);
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekdiskon = "SELECT * FROM produkpromo WHERE provinsi='$provinsi' && bidangusaha='$bidangusaha' && posisi='diskon' && status='on' ORDER BY id_produkpromo ASC";
$konekdiskon = mysql_query($query_konekdiskon, $koneksi) or die(mysql_error());
$row_konekdiskon = mysql_fetch_assoc($konekdiskon);
$totalRows_konekdiskon = mysql_num_rows($konekdiskon);
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekwisjateng = "SELECT * FROM wisatadiy where provinsi='$provinsi' && jenis='photo' && status='on' ORDER BY id_wisatadiy DESC";
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekinformasivid = "SELECT * FROM info WHERE provinsi= '$provinsi' && bidangusaha= '$bidangusaha' && jenis= 'video' && status= '$status' ORDER BY id_info DESC";
...
mysql_select_db($database_koneksi, $koneksi);
$query_konekwisjatengvid = "SELECT * FROM wisatadiy where provinsi='$provinsi' && jenis='video' && status='on' ORDER BY id_wisatadiy DESC";
...
$query_konekwisataindonesia = "SELECT * FROM wisatadiy where provinsi = '$provinsi' and status='on' && kategori='wisata indonesia' ORDER BY id_wisatadiy DESC";
